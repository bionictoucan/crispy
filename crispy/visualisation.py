...
class SpectralViewer:
    ...
    def __init__(self, data, wcs=None, uncertainty=None, mask=None, nonu=False):
        ...
        if not nonu:
            if isinstance(data, str):
                ...
                if self.cube.file.data.ndim == 3:
                    self.wvls = (
                        self.cube.wave(np.arange(self.cube.shape[0])) << u.Angstrom
                    )
                ...
                if self.cube.list[0].file.data.ndim == 3:
                    self.wvls1 = (
                        self.cube.list[0].wave(np.arange(self.cube.list[0].shape[0]))
                        << u.Angstrom
                    )
                ...
                if self.cube.list[1].file.data.ndim == 3:
                    self.wvls2 = (
                        self.cube.list[1].wave(np.arange(self.cube.list[1].shape[0]))
                        << u.Angstrom
                    )
        ...
        if isinstance(self.cube, CRISP) or isinstance(self.cube, CRISPNonU):
            ...
            try:
                self.ax1 = self.fig.add_subplot(
                    1, 2, 1, projection=self.cube.wcs.dropaxis(-1)
                )
            except:
                self.ax1 = self.fig.add_subplot(
                    1,
                    2,
                    1,
                    projection=SlicedLowLevelWCS(self.cube[0].wcs.low_level_wcs, 0),
                )
            ...
            ll = widgets.SelectionSlider(
                options=[
                    np.round(l - np.median(self.wvls), decimals=2).value
                    for l in self.wvls
                ],
                description=f"{self.D} {self.l} [{self.aa}]",
            )
            ...
            try:
                self.ax1 = self.fig.add_subplot(
                    2, 2, 1, projection=self.cube.list[0].wcs.dropaxis(-1)
                )
            except:
                self.ax1 = self.fig.add_subplot(
                    2,
                    2,
                    1,
                    projection=SlicedLowLevelWCS(
                        self.cube.list[0][0].wcs.low_level_wcs, 0
                    ),
                )
            self.ax1.set_ylabel("Helioprojective Latitude [arcsec]")
            self.ax1.set_xlabel("Helioprojective Longitude [arcsec]")
            ...
            try:
                self.ax2 = self.fig.add_subplot(
                    2, 2, 3, projection=self.cube.list[0].wcs.dropaxis(-1)
                )
            except:
                self.ax2 = self.fig.add_subplot(
                    2,
            ...
            ll1 = widgets.SelectionSlider(
                options=[
                    np.round(l - np.median(self.wvls1), decimals=2).value
                    for l in self.wvls1
                ],
                description=rf"{self.D} {self.l}$_{1}$ [{self.aa}]",
                style={"description_width": "initial"},
            )
            ll2 = widgets.SelectionSlider(
                options=[
                    np.round(l - np.median(self.wvls2), decimals=2).value
                    for l in self.wvls2
                ],
                description=rf"{self.D} {self.l}$_{2}$ [{self.aa}]",
                style={"description_width": "initial"},
            )
        ...
        self.receiver = self.fig.canvas.mpl_connect(
            "button_press_event", self._on_click
        )

        try:
            x = widgets.IntText(
                value=1, min=1, max=self.cube.shape[-1], description="x [pix]"
            )
            y = widgets.IntText(
                value=1, min=1, max=self.cube.shape[-2], description="y [pix]"
            )
        except:
            x = widgets.IntText(
                value=1, min=1, max=self.cube.list[0].shape[-1], description="x [pix]"
            )
            y = widgets.IntText(
                value=1, min=1, max=self.cube.list[0].shape[-2], description="y [pix]"
            )
        ...
        if isinstance(self.cube, CRISP) or isinstance(self.cube, CRISPNonU):
            if self.shape == "point":
                ...
                circ = patches.Circle(
                    centre_coord[::-1],
                    radius=10,
                    facecolor=list(pt_bright_cycler)[self.colour_idx]["color"],
                    edgecolor="k",
                    linewidth=1,
                )
                ...
                txt = self.ax1.text(
                    centre_coord[1] + 20,
                    centre_coord[0] + 10,
                    s=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
                    fontdict=font,
                )
                ...
                if self.cube.file.data.ndim == 3:
                    self.ax2.plot(
                        self.wvls,
                        self.cube.file.data[:, centre_coord[0], centre_coord[1]],
                        marker=Line2D.filled_markers[
                            self.colour_idx + self.n * len(pt_bright_cycler)
                        ],
                        label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
                        c=list(pt_bright_cycler)[self.colour_idx]["color"],
                    )
                elif self.cube.file.data.ndim == 4:
                    self.ax2.plot(
                        self.wvls,
                        self.cube.file.data[0, :, centre_coord[0], centre_coord[1]],
                        marker=Line2D.filled_markers[
                            self.colour_idx + self.n * len(pt_bright_cycler)
                        ],
                        label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
                ...
                rect = patches.Rectangle(
                    ...
                    self.boxy,
                    linewidth=2,
                    edgecolor=list(pt_bright_cycler)[self.colour_idx]["color"],
                    facecolor="none",
                )
            ...
            if self.shape == "point":
                if self.colour_idx > len(pt_bright_cycler) - 1:
                    self.colour_idx = 0
                    self.n += 1
                centre_coord = int(event.ydata), int(
                    event.xdata
                )  # with WCS, the event data is returned in pixels so we don't need to do the conversion from real world but rather to real world later on
                self.px_coords.append(centre_coord)
                ...
                txt_1.set_path_effects(
                    [PathEffects.withStroke(linewidth=3, foreground="k")]
                )
                txt_2.set_path_effects(
                    [PathEffects.withStroke(linewidth=3, foreground="k")]
                )
                ...
                if self.cube.list[0].file.data.ndim == 3:
                    ...
                    self.ax3.plot(
                        self.wvls1,
                        self.cube.list[0].file.data[
                            0, :, centre_coord[0], centre_coord[1]
                        ],
                        marker=Line2D.filled_markers[
                            self.colour_idx + self.n * len(pt_bright_cycler)
                        ],
    ...
    def _img_plot1(self, ll):
        ...
        ll_idx = int(
            np.where(
                np.round(self.wvls, decimals=2).value
                == np.round(np.median(self.wvls).value + ll, decimals=2)
            )[0]
        )
        ...
        self.ax1.set_title(rf"{el} {self.aa} {self.D} {self.l}$_{1}$ = {ll} {self.aa}")
        self.fig.colorbar(
            im1, ax=self.ax1, orientation="horizontal", label="Intensity [DNs]"
        )
    ...
    def _img_plot2(self, ll1, ll2):
        ...
        ll1_idx = int(
            np.where(
                np.round(self.wvls1, decimals=2).value
                == np.round(np.median(self.wvls1).value + ll1, decimals=2)
            )[0]
        )
        ll2_idx = int(
            np.where(
                np.round(self.wvls2, decimals=2).value
                == np.round(np.median(self.wvls2).value + ll2, decimals=2)
            )[0]
        )
        ...
        self.ax1.set_title(
            rf"{el1} {self.aa} {self.D} {self.l}$_{1}$ = {ll1} {self.aa}"
        )
        self.ax2.set_title(
            rf"{el2} {self.aa} {self.D} {self.l}$_{2}$ = {ll2} {self.aa}"
        )
        self.fig.colorbar(
            im1, ax=self.ax1, orientation="horizontal", label="Intensity [DNs]"
        )
        self.fig.colorbar(
            im2, ax=self.ax2, orientation="horizontal", label="Intensity [DNs]"
        )
...
class WidebandViewer:
    ...
    def __init__(self, files):
        ...
        if type(self.cube) is not list:
            try:
                self.time = [
                    date2num(f.file.header["DATE-AVG"]) for f in self.cube.list
                ]
            except KeyError:
                self.time = [
                    date2num(
                        f.file.header["date_obs"] + " " + f.file.header["time_obs"]
                    )
                    for f in self.cube.list
                ]
            self.fig = plt.figure(figsize=(8, 10))
            ...
            try:
                self.time1 = [
                    date2num(f.file.header["DATE-AVG"]) for f in self.cube[0].list
                ]
                self.time2 = [
                    date2num(f.file.header["DATE-AVG"]) for f in self.cube[1].list
                ]
            except KeyError:
                self.time1 = [
                    date2num(
            ...
            self.fig = plt.figure(figsize=(8, 10))
            self.ax1 = self.fig.add_subplot(
                2, 2, 1, projection=self.cube[0].list[0].wcs
            )
            self.ax1.set_ylabel("Helioprojective Latitude [arcsec]")
            self.ax1.set_xlabel("Helioprojective Longitude [arcsec]")
            self.ax1.xaxis.set_label_position("top")
            ...
            self.ax2 = self.fig.add_subplot(
                2, 2, 3, projection=self.cube[1].list[0].wcs
            )
            ...
            t1 = widgets.IntSlider(
                value=0,
                min=0,
                max=len(self.cube[0].list) - 1,
                step=1,
                description="Time index: ",
                style={"description_width": "initial"},
            )
            t2 = widgets.IntSlider(
                value=0,
                min=0,
                max=len(self.cube[1].list) - 1,
                step=1,
                description="Time index: ",
                style={"description_width": "initial"},
            )
        ...
        self.reveiver = self.fig.canvas.mpl_connect(
            "button_press_event", self._on_click
        )
    ...
    def _on_click(self, event):
        ...
        if isinstance(self.cube, CRISPWidebandSequence):
            if self.shape == "point":
                ...
                px = (
                    self.cube.list[0].wcs.array_index_to_world(*centre_coord)
                    << u.arcsec
                )
                prof = [
                    f.file.data[centre_coord[0], centre_coord[1]]
                    for f in self.cube.list
                ]
                self.ax2.plot(
                    self.time,
            ...
            if self.shape == "point":
                ...
                px = (
                    self.cube[0].list[0].wcs.array_index_to_world(*centre_coord)
                    << u.arcsec
                )
                prof_1 = [
                    f.file.data[centre_coord[0], centre_coord[1]]
...
class AtmosViewer:
    ...
    def __init__(self, filename, z=None, wcs=None, header=None, eb=False):
        ...
        self.ax1 = self.fig.add_subplot(
            self.gs[:2, 0], projection=self.inv.wcs.dropaxis(-1)
        )
        self.ax2 = self.fig.add_subplot(
            self.gs[:2, 1], projection=self.inv.wcs.dropaxis(-1)
        )
        self.ax3 = self.fig.add_subplot(
            self.gs[:2, 2], projection=self.inv.wcs.dropaxis(-1)
        )
        self.ax1.set_ylabel("Helioprojective Latitude [arcsec]")
        self.ax1.set_xlabel("Helioprojective Longitude [arcsec]")
        self.ax2.set_xlabel("Helioprojective Longitude [arcsec]")
        ...
        widgets.interact(
            self._img_plot,
            z=widgets.SelectionSlider(
                options=np.round(self.inv.z, decimals=3),
                description="Image height [Mm]: ",
                style={"description_width": "initial"},
                layout=widgets.Layout(width="50%"),
            ),
        )
        ...
        x = widgets.IntText(
            value=1, min=1, max=self.inv.ne.shape[-1], description="x [pix]"
        )
        y = widgets.IntText(
            value=1, min=1, max=self.inv.ne.shape[-2], description="y [pix]"
        )
        outx = widgets.interactive_output(self._boxx, {"x": x})
        outy = widgets.interactive_output(self._boxy, {"y": y})
        display(widgets.HBox([x, y]))
    ...
    def _on_click(self, event):
        ...
        if self.shape == "point":
            ...
            txt_3.set_path_effects(
                [PathEffects.withStroke(linewidth=3, foreground="k")]
            )
            if self.eb:
                self.ax4.errorbar(
                    self.inv.z,
                    self.inv.ne[:, centre_coord[0], centre_coord[1]],
                    yerr=self.inv.err[:, centre_coord[0], centre_coord[1], 0],
                    marker=Line2D.filled_markers[
                        self.colour_idx + self.n * len(pt_bright_cycler)
                    ],
                    label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
                ...
                self.ax5.errorbar(
                    self.inv.z,
                    self.inv.temp[:, centre_coord[0], centre_coord[1]],
                    yerr=self.inv.err[:, centre_coord[0], centre_coord[1], 1],
                    marker=Line2D.filled_markers[
                        self.colour_idx + self.n * len(pt_bright_cycler)
                    ],
                    label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
                ...
                self.ax6.errorbar(
                    self.inv.z,
                    self.inv.vel[:, centre_coord[0], centre_coord[1]],
                    yerr=self.inv.err[:, centre_coord[0], centre_coord[1], 2],
                    marker=Line2D.filled_markers[
                        self.colour_idx + self.n * len(pt_bright_cycler)
                    ],
                    label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
    ...
    def _img_plot(self, z):
        ...
        self.fig.colorbar(
            im1, ax=self.ax1, orientation="horizontal", label=r"log $n_{e}$ [cm$^{-3}$]"
        )

        im2 = self.ax2.imshow(self.inv.temp[z_idx], cmap="hot")
        self.fig.colorbar(
            im2, ax=self.ax2, orientation="horizontal", label=r"log T [K]"
        )

        im3 = self.ax3.imshow(
            self.inv.vel[z_idx],
            cmap="RdBu",
            clim=(-np.max(self.inv.vel[z_idx]), np.max(self.inv.vel[z_idx])),
        )
        self.fig.colorbar(
            im3, ax=self.ax3, orientation="horizontal", label=r"v [km s$^{-1}$]"
        )
...
class ImageViewer:
    ...
    def __init__(self, data, wcs=None, uncertainty=None, mask=None, nonu=False):
        ...
        if isinstance(self.cube, CRISP) or isinstance(self.cube, CRISPNonU):
            ...
            try:
                self.ax1 = self.fig.add_subplot(
                    1, 1, 1, projection=self.cube.wcs.dropaxis(-1)
                )
            except:
                self.ax1 = self.fig.add_subplot(
                    1,
            ...
            try:
                self.ax1 = self.fig.add_subplot(
                    1, 2, 1, projection=self.cube.list[0].wcs.dropaxis(-1)
                )
            except:
                self.ax1 = self.fig.add_subplot(
                    1,
            ...
            try:
                self.ax2 = self.fig.add_subplot(
                    1, 2, 2, projection=self.cube.list[1].wcs.dropaxis(-1)
                )
            except:
                self.ax2 = self.fig.add_subplot(
                    1,
                    ...
                    2,
                    projection=SlicedLowLevelWCS(
                        self.cube.list[1][0].wcs.low_level_wcs, 0
                    ),
                )
            self.ax2.set_ylabel("Helioprojective Latitude [arcsec]")
            self.ax2.set_xlabel("Helioprojective Longitude [arcsec]")
...
class SpectralTimeViewer:
    ...
    def __init__(
        self, data1, data2=None, wcs=None, uncertainty=None, mask=None, nonu=False
    ):
        plt.style.use("bmh")
        self.aa = html.unescape("&#8491;")
        self.l = html.unescape("&lambda;")
            ...
            if isinstance(data1, list):
                ...
                if self.cube1.list[0].file.data.ndim == 3:
                    self.wvls1 = self.cube1.list[0].wave(
                        np.arange(self.cube1.list[0].shape[0])
                    )
                elif self.cube1.list[0].file.data.ndim == 4:
                    self.wvls1 = self.cube1.list[0].wave(
                        np.arange(self.cube1.list[0].shape[1])
                    )
            if data2 is None:
                pass
                ...
                if self.cube2.list[0].file.data.ndim == 3:
                    self.wvls2 = self.cube2.list[0].wave(
                        np.arange(self.cube2.list[0].shape[0])
                    )
        ...
        if data2 is None:
            ...
            if self.cube1.list[0].file.data.ndim == 3:
                self.ax1 = self.fig.add_subplot(
                    self.gs[0, 0], projection=self.cube1.list[0].wcs.dropaxis(-1)
                )
            elif self.cube1.list[0].file.data.ndim == 4:
                self.ax1 = self.fig.add_subplot(
                    self.gs[0, 0],
                    projection=SlicedLowLevelWCS(
                        self.cube1.list[0][0].wcs.low_level_wcs, 0
                    ),
                )
            self.ax1.set_ylabel("Helioprojective Latitude [arcsec]")
            self.ax1.set_xlabel("Helioprojective Longitude [arcsec]")
            ...
            self.ll = widgets.SelectionSlider(
                options=[
                    np.round(l - np.median(self.wvls1), decimals=2).value
                    for l in self.wvls1
                ],
            ...
            self.t = widgets.IntSlider(
                value=0,
                min=0,
                max=len(self.cube1.list) - 1,
                step=1,
                description="Time index: ",
                disabled=False,
            )
            ...
            try:
                self.times1 = [
                    date2num(f.file.header["DATE-AVG"]) for f in self.cube1.list
                ]
            except KeyError:
                self.times1 = [
                    date2num(
                    ...
                    for f in self.cube1.list
                ]

            out1 = widgets.interactive_output(
                self._img_plot1, {"ll": self.ll, "t": self.t}
            )
            out2 = widgets.interactive_output(self._shape, {"opts": shape})
            ...
            try:
                self.ax2 = self.fig.add_subplot(
                    self.gs[1, 0], projection=self.cube2.list[0].wcs.dropaxis(-1)
                )
            except:
                self.ax2 = self.fig.add_subplot(
                    self.gs[1, 0],
                    projection=SlicedLowLevelWCS(
                        self.cube2.list[0][0].wcs.low_level_wcs, 0
                    ),
                )
            self.ax2.set_ylabel("Helioprojective Latitude [arcsec]")
            self.ax2.set_xlabel("Helioprojective Longitude [arcsec]")
            ...
            self.ll1 = widgets.SelectionSlider(
                ...
                description=rf"{self.aa} {self.D} {self.l}$_{1}$ [{self.aa}]",
                style={"description_width": "initial"},
            )
            self.ll2 = widgets.SelectionSlider(
                ...
                description=rf"{self.aa} {self.D} {self.l}$_{2}$ [{self.aa}]",
                style={"description_width": "initial"},
            )
            ...
            self.t1 = widgets.IntSlider(
                ...
                step=1,
                disabled=False,
                description=r"t$_{1}$ index: ",
            )
            self.t2 = widgets.IntSlider(
                value=0,
                min=0,
                max=len(self.cube2.list) - 1,
                step=1,
                disabled=False,
                description=r"t$_{2}$ index: ",
            )
            ...
            try:
                ...
                self.times2 = [
                    date2num(f.file.header["DATE-AVG"]) for f in self.cube2.list
                ]
            except KeyError:
                self.times1 = [
                    date2num(
                ...
                self.times2 = [
                    ...
                    for f in self.cube2.list
                ]

            out1 = widgets.interactive_output(
                self._img_plot2,
                {"ll1": self.ll1, "ll2": self.ll2, "t1": self.t1, "t2": self.t2},
            )
            out2 = widgets.interactive_output(self._shape, {"opts": shape})
            ...
            display(
                widgets.HBox(
                    [
                        widgets.VBox(
                            [
                                widgets.HBox([self.ll1, self.ll2]),
                                widgets.HBox([self.t1, self.t2]),
                            ]
                        ),
                        shape,
                    ]
        ...
        x = widgets.IntText(
            value=1, min=1, max=self.cube1.list[0].shape[-1], description="x [pix]"
        )
        y = widgets.IntText(
            value=1, min=1, max=self.cube1.list[0].shape[-2], description="y [pix]"
        )
        outx = widgets.interactive_output(self._boxx, {"x": x})
        outy = widgets.interactive_output(self._boxy, {"y": y})
        display(widgets.HBox([x, y]))
        ...
        if not hasattr(self, "cube2"):
            if self.shape == "point":
                ...
                if self.cube1.list[0].file.data.ndim == 3:
                    self.ax2.plot(
                        self.wvls1,
                        self.cube1.list[self.t.value].file.data[
                            :, centre_coord[0], centre_coord[1]
                        ],
                        marker=Line2D.filled_markers[
                            self.colour_idx + self.n * len(pt_bright_cycler)
                ...
                ll_idx = int(
                    np.where(
                        np.round(self.wvls1, decimals=2).value
                        == np.round(
                            np.median(self.wvls1).value + self.ll.value, decimals=2
                        )
                    )[0]
                )
                if self.cube1.list[0].file.data.ndim == 3:
                    i_time1 = [
                        f.file.data[ll_idx, centre_coord[0], centre_coord[1]]
                        for f in self.cube1.list
                    ]
                    self.ax3.plot(
                        self.times1,
                    ...
                    i_time1 = [
                        f.file.data[0, ll_idx, centre_coord[0], centre_coord[1]]
                        for f in self.cube1.list
                    ]
                    self.ax3.plot(
                        self.times1,
                ...
                self.coords.append(
                    self.cube1.list[0].to_lonlat(*box_anchor) << u.arcsec
                )
                rect = patches.Rectangle(
                    box_anchor[::-1],
                    self.boxx,
            ...
            if self.shape == "point":
                ...
                if self.cube1.list[0].file.data.ndim == 3:
                    self.ax3.plot(
                        self.wvls1,
                        self.cube1.list[self.t1.value].file.data[
                            :, centre_coord[0], centre_coord[1]
                        ],
                        marker=Line2D.filled_markers[
                            self.colour_idx + self.n * len(pt_bright_cycler)
                ...
                if self.cube2.list[0].file.data.ndim == 3:
                    self.ax4.plot(
                        self.wvls2,
                        self.cube2.list[self.t2.value].file.data[
                            :, centre_coord[0], centre_coord[1]
                        ],
                        marker=Line2D.filled_markers[
                            self.colour_idx + self.n * len(pt_bright_cycler)
                ...
                ll_idx1 = int(
                    np.where(
                        np.round(self.wvls1, decimals=2).value
                        == np.round(
                            np.median(self.wvls1).value + self.ll1.value, decimals=2
                        )
                    )[0]
                )
                ll_idx2 = int(
                    np.where(
                        np.round(self.wvls2, decimals=2).value
                        == np.round(
                            np.median(self.wvls2).value + self.ll2.value, decimals=2
                        )
                    )[0]
                )
                if self.cube1.list[0].file.data.ndim == 3:
                    i_time1 = [
                        f.file.data[ll_idx1, centre_coord[0], centre_coord[1]]
                        for f in self.cube1.list
                    ]
                elif self.cube1.list[0].file.data.ndim == 4:
                    i_time1 = [
                        f.file.data[0, ll_idx1, centre_coord[0], centre_coord[1]]
                        for f in self.cube1.list
                    ]
                if self.cube2.list[0].file.data.ndim == 3:
                    i_time2 = [
                        f.file.data[ll_idx2, centre_coord[0], centre_coord[1]]
                        for f in self.cube2.list
                    ]
                elif self.cube2.list[0].file.data.ndim == 4:
                    i_time2 = [
                        f.file.data[0, ll_idx2, centre_coord[0], centre_coord[1]]
                        for f in self.cube2.list
                    ]
                self.ax5.plot(
                    self.times1,
    ...
    def _img_plot1(self, ll, t):
        ...
        ll_idx = int(
            np.where(
                np.round(self.wvls1, decimals=2).value
                == np.round(np.median(self.wvls1).value + ll, decimals=2)
            )[0]
        )
    ...
    def _img_plot2(self, ll1, ll2, t1, t2):
        ...
        self.fig.colorbar(
            im1, ax=self.ax1, orientation="vertical", label="Intensity [DNs]"
        )
        self.fig.colorbar(
            im2, ax=self.ax2, orientation="vertical", label="Intensity [DNs]"
        )
...
class PolarimetricViewer:
    ...
    def __init__(self, data, wcs=None, uncertainty=None, mask=None, nonu=False):
        ...
        ll = widgets.SelectionSlider(
            options=[
                np.round(l - np.median(self.wvls), decimals=2).value for l in self.wvls
            ],
            description=f"{self.D} {self.l} [{self.aa}]",
        )
    ...
    def _on_click(self, event):
        ...
        if self.shape == "point":
            ...
            self.ax3.plot(
                self.wvls,
                self.cube.file.data[1, :, centre_coord[0], centre_coord[1]],
                marker=Line2D.filled_markers[
                    self.colour_idx + self.n * len(pt_bright_cycler)
                ],
                label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
            ...
            self.ax4.plot(
                self.wvls,
                self.cube.file.data[2, :, centre_coord[0], centre_coord[1]],
                marker=Line2D.filled_markers[
                    self.colour_idx + self.n * len(pt_bright_cycler)
                ],
                label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
            ...
            self.ax5.plot(
                self.wvls,
                self.cube.file.data[3, :, centre_coord[0], centre_coord[1]],
                marker=Line2D.filled_markers[
                    self.colour_idx + self.n * len(pt_bright_cycler)
                ],
                label=f"{self.colour_idx+1+(self.n*len(pt_bright_cycler))}",
    ...
    def _img_plot1(self, ll, s):
        ...
        if s_idx == 0:
            ...
            im1 = self.ax1.imshow(
                self.cube.file.data[s_idx, ll_idx], cmap="Greys_r", vmin=-10, vmax=10
            )
        else:
            im1 = self.ax1.imshow(
                self.cube.file.data[s_idx, ll_idx], cmap="Greys_r", vmin=-50, vmax=50
            )
        try:
            el = self.cube.file.header["WDESC1"]
        except KeyError:
...
class PolarimetricTimeViewer:
    ...
    def __init__(self, data, wcs=None, uncertainty=None, mask=None, nonu=False):
        ...
        self.ax1 = self.fig.add_subplot(
            self.gs[:2, :2],
            projection=SlicedLowLevelWCS(self.cube.list[0][0].wcs.low_level_wcs, 0),
        )
        self.ax1.set_ylabel("Helioprojective Latitude [arcsec]")
        self.ax1.set_xlabel("Helioprojective Longitude [arcsec]")
        ...
        try:
            ...
            self.times1 = [
                date2num(f.file.header["date_obs"] + " " + f.file.header["time_obs"])
                for f in self.cube.list
            ]

        out1 = widgets.interactive_output(
            self._img_plot1, {"ll": self.ll, "s": s, "t": self.t}
        )
        out2 = widgets.interactive_output(self._shape, {"opts": shape})

        display(
            widgets.HBox([widgets.VBox([self.ll, s]), widgets.VBox([self.t, shape])])
        )
    ...
    def _on_click(self, event):
        ...
        if self.shape == "point":
            ...
            self.ax2.plot(
                self.wvls,
                self.cube.list[self.t.value].file.data[
                    0, :, centre_coord[0], centre_coord[1]
                ],
                marker=Line2D.filled_markers[
                    self.colour_idx + self.n * len(pt_bright_cycler)
            ...
            self.ax3.plot(
                self.wvls,
                self.cube.list[self.t.value].file.data[
                    1, :, centre_coord[0], centre_coord[1]
                ],
                marker=Line2D.filled_markers[
                    self.colour_idx + self.n * len(pt_bright_cycler)
                ],
            ...
            self.ax4.plot(
                self.wvls,
                self.cube.list[self.t.value].file.data[
                    2, :, centre_coord[0], centre_coord[1]
                ],
                marker=Line2D.filled_markers[
                    self.colour_idx + self.n * len(pt_bright_cycler)
                ],
            ...
            self.ax5.plot(
                self.wvls,
                self.cube.list[self.t.value].file.data[
                    3, :, centre_coord[0], centre_coord[1]
                ],
                marker=Line2D.filled_markers[
                    self.colour_idx + self.n * len(pt_bright_cycler)
                ],
            ...
            ll_idx = int(
                np.where(
                    np.round(self.wvls, decimals=2).value
                    == np.round(np.median(self.wvls).value + self.ll.value, decimals=2)
                )[0]
            )
            i_time = [
                f.file.data[0, ll_idx, centre_coord[0], centre_coord[1]]
            ...
            q_time = [
                f.file.data[1, ll_idx, centre_coord[0], centre_coord[1]]
                for f in self.cube.list
            ]
            u_time = [
                f.file.data[2, ll_idx, centre_coord[0], centre_coord[1]]
                for f in self.cube.list
            ]
            v_time = [
                f.file.data[3, ll_idx, centre_coord[0], centre_coord[1]]
                for f in self.cube.list
            ]
            self.ax6.plot(
                self.times1,
    ...
    def _img_plot1(self, ll, s, t):
        ...
        if s_idx == 0:
            ...
            im1 = self.ax1.imshow(
                self.cube.list[t].file.data[s_idx, ll_idx],
                cmap="Greys_r",
                vmin=-10,
                vmax=10,
            )

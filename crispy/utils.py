...
def CRISP_sequence_constructor(
...
def rotate_crop_aligned_data(imA, imB):
    ...
    if len(imA.shape) == 4 and len(imB.shape) == 4:
        ...
        raise ValueError(
            "Unexpected dimensionality of imA and imB, expected both 3 or 4, got %d and %d"
    ...
    rotMat = np.array(
        (
            (np.cos(-angle), np.sin(-angle), imCentre[0]),
    ...
    for wave in range(imA.shape[0]):
        imARot[wave] = rotate(
            imA[wave], -np.rad2deg(angle), cval=imA[wave, 0, 0], reshape=False
        )

    imBRot = np.empty_like(imB)
    for wave in range(imB.shape[0]):
        imBRot[wave] = rotate(
            imB[wave], -np.rad2deg(angle), cval=imB[wave, 0, 0], reshape=False
        )
...
def rotate_crop_data(im):
    ...
    if len(im.shape) == 4:
        ...
        raise ValueError(
            "Unexpected dimensionality of im, expected 3 or 4, got %d" % (len(im.shape))
        )
    ...
    for wave in range(im.shape[0]):
        imRot[wave] = rotate(
            im[wave], -np.rad2deg(angle), cval=im[wave, 0, 0], reshape=False
        )
...
def reconstruct_full_frame(cropData, im):
    ...
    for wave in range(wvls):
        imFFRot[wave] = rotate(
            imFullFrame[wave], np.rad2deg(cropData["angle"]), reshape=False
        )
...
def segment_cube(img_cube, n):
    ...
    segments = np.swapaxes(
        segments, 0, 1
    )  # this puts the segment dimension first, wavelength second to make it easier for data loaders
    ...
    for j in y_range:
        for i in x_range:
            if i != x_range[-1] and j != y_range[-1]:
                ...
                mosaic_img[j * n : (j + 1) * n, -x_overlap:] = segments[
                    j, i, :, -x_overlap:
                ]
            elif i != x_range[-1] and j == y_range[-1]:
                mosaic_img[-y_overlap:, i * n : (i + 1) * n] = segments[
                    j, i, -y_overlap:
                ]
            elif i == x_range[-1] and j == y_range[-1]:
                mosaic_img[-y_overlap:, -x_overlap:] = segments[
                    j, i, -y_overlap:, -x_overlap:
                ]
            else:
                raise IndexError(
                    "These indices are out of the bounds of the image. Check your ranges!"
                )
        ...
        for i in x_range:
            if (
                ...
                for k in range(top.shape[-1]):
                    top_new[:, k] = np.interp(
                        np.arange(top.shape[0]),
                        np.array([0, top.shape[0] - 1]),
                        np.array([top[0, k], top[-1, k]]),
                    )
                    bottom_new[:, k] = np.interp(
                        np.arange(bottom.shape[0]),
                        np.array([0, bottom.shape[0] - 1]),
                        np.array([bottom[0, k], bottom[-1, k]]),
                    )
                    left_new[k, :] = np.interp(
                        np.arange(left.shape[1]),
                        np.array([0, left.shape[1] - 1]),
                        np.array([left[k, 0], left[k, -1]]),
                    )
                    right_new[k, :] = np.interp(
                        np.arange(right.shape[1]),
                        np.array([0, right.shape[1] - 1]),
                        np.array([right[k, 0], right[k, -1]]),
                    )
...
def mosaic_cube(segments, img_shape, n):
    ...
    segments = np.swapaxes(
        segments, 0, 1
    )  # swap the number of segments and wavelength channels back to make it easier to mosaic along the wavelength axis
